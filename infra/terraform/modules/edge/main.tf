...
resource "aws_cloudfront_origin_access_control" "assets" {
  ...
  description                       = "Access control for public asset prefixes"
  origin_access_control_origin_type = "s3"
  signing_behavior                  = "always"
  signing_protocol                  = "sigv4"
}
...
data "aws_iam_policy_document" "user_assets" {
  statement {
    sid    = "AllowCloudFrontReadPublicAssetPrefixes"
    effect = "Allow"
...
resource "aws_s3_bucket_policy" "user_assets" {
  bucket = var.user_asset_bucket_id
  policy = data.aws_iam_policy_document.user_assets.json
}

resource "aws_wafv2_web_acl" "alb" {
  count = var.waf_enable_rate_limit ? 1 : 0
...
resource "aws_cloudfront_distribution" "assets" {
  enabled = true
  aliases = [var.asset_domain_name]

  origin {
    domain_name              = var.user_asset_bucket_regional_domain_name
    origin_id                = "user-assets-s3"
  ...
  default_cache_behavior {
    allowed_methods        = ["GET", "HEAD", "OPTIONS"]
    cached_methods         = ["GET", "HEAD"]
    target_origin_id       = "user-assets-s3"
    viewer_protocol_policy = "redirect-to-https"
    compress               = true
